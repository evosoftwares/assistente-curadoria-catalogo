...
|---|---|---|
| Feedback 👍/👎 + comentário por resposta | **✅ COLETOR + CONSUMIDOR INSTALADOS**: UI/MCP → `POST /feedback` → `data/feedback.jsonl`; `eval/build_gold_from_feedback.py` agrega em **gold-set vivo** (`gold_live.json`, schema compatível com `gold.json`) com taxa de aceitação por pergunta e fila de revisão dos 👎. Resta MADURAR (volume real) e plugar no `retrieval_metrics` | método anti-circular já em `eval/build_gold.py` |
